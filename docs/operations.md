...
This guide covers the v1.0.7 Windows/Ubuntu desktop node, headless CLI, and
...
- `entcoin_1.0.7_amd64.deb`: desktop application and CLI installer;
...
v1.0.7 binaries are not code-signed, so a checksum proves only that the file matches
...
Install Ubuntu packages with `sudo apt install ./entcoin_1.0.7_amd64.deb`, then
...
Release. Entcoin selects the platform installer, limits metadata and download
sizes, accepts only trusted GitHub HTTPS download hosts, and verifies the exact
asset against `SHA256SUMS.txt` or `SHA256SUMS-linux.txt`. It then opens the
Windows installer or Ubuntu package installer for explicit approval. The
desktop never installs an update silently or bypasses SmartScreen, package
manager authorization, or other operating-system trust prompts.
...
testnet chain. There is intentionally no CLI restore command in v1.0.7; start
...
Then start v1.0.7 normally and restore the phrase or `.entwallet` from the

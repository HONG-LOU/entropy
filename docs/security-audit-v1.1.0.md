...
| ENT-2026-03 | Medium | GitHub Actions used mutable major-version tags and release build jobs inherited repository write permission. | Every Action is pinned to a full commit SHA. Build jobs have read-only contents permission; only publish receives contents, OIDC, and attestation writes. |

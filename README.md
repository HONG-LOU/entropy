...
Entcoin v1.0.8 is a compact proof-of-work mainnet packaged as a Windows and
...
needed. The current v1.0.8 release is unsigned, so Windows SmartScreen may show
...
```bash
sudo apt install ./entcoin_1.0.8_amd64.deb
entcoin
```
...
`.entwallet` backup. Start v1.0.8 to create the mainnet directory, then use the

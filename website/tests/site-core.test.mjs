...
test("release selection accepts only known stable release assets", () => {
  const selected = selectReleaseAssets({
    tag_name: "v1.0.13",
    html_url: "https://github.com/HONG-LOU/entcoin/releases/tag/v1.0.13",
    draft: false,
    prerelease: false,
    assets: [
      asset("Entcoin.exe"),
      asset("entcoin-amd64-installer.exe"),
      asset("entcoin_1.0.13_amd64.deb"),
  ...
  for (const address of [selected.windowsPortable, selected.windowsInstaller, selected.ubuntu, selected.linuxCli, selected.windowsCli]) {
    assert.match(address, /^https:\/\/entcoin\.xyz\/downloads\/v1\.0\.13\//);
  }
  assert.equal(selected.release, "https://github.com/HONG-LOU/entcoin/releases/tag/v1.0.13");
});

test("release selection falls back for drafts, prereleases, and foreign URLs", () => {
  const draft = selectReleaseAssets({ tag_name: "v1.0.13", draft: true, prerelease: false, assets: [] });
...
test("homepage translation keys are all defined", async () => {
  ...
  for (const assetName of [
    ...
    "Entcoin.exe",
    "entcoin_1.0.13_amd64.deb",
    "entcoin-cli-linux-amd64",
    "entcoin-cli.exe",
    "SHA256SUMS.txt",
  ]) {
    assert.ok(html.includes(`/downloads/v1.0.13/${assetName}`), assetName);
  }
...
test("desktop update fallback names the current official release", async () => {
  ...
  assert.deepEqual(manifest, {
    ...
    release_url: "https://github.com/HONG-LOU/entcoin/releases/tag/v1.0.13",
  });
});
...
function asset(name) {
  return {
    name,
    browser_download_url: `https://github.com/HONG-LOU/entcoin/releases/download/v1.0.13/${name}`,
  };
}
